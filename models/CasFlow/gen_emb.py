...
def sequence2list(filename):
    ...
    with open(filename, 'r') as f:
        for line in f:
            paths = line.strip().split('\t')[:-1][:opts.max_seq + 1]
            graphs[paths[0]] = list()
...
def write_cascade(graphs, labels, id2row, filename, gg_emb, weight=True):
    ...
    for key, graph in graphs.items():
        ...
        d = opts.cg_emb_dim / (2 * opts.num_s)
        if opts.cg_emb_dim % 4 != 0:
            raise ValueError
        ...
        if weight:
            cascade_embedding = np.concatenate([np.reshape(times, (-1, 1)),
                                               ...
                                               axis=1)
        # cascade_embedding = sorted(cascade_embedding,key=lambda x:x[0],reverse=True)
...
def main(argv=None):
    ...
    graph_train = sequence2list(opts.DATA_PATH + 'train.txt')
    graph_val = sequence2list(opts.DATA_PATH + 'val.txt')
    graph_test = sequence2list(opts.DATA_PATH + 'test.txt')

    # get the information of labels of cascades
    label_train = read_labels(opts.DATA_PATH + 'train.txt')
    label_val = read_labels(opts.DATA_PATH + 'val.txt')
    label_test = read_labels(opts.DATA_PATH + 'test.txt')

    # load global graph and generate id2row
    with open(opts.DATA_PATH + opts.gg_path, 'rb') as f:
        gg = pickle.load(f)

    # sparse matrix factorization
    model = SparseMatrixFactorization(gg, opts.gg_emb_dim)
    ...
    write_cascade(graph_train, label_train, id2row, opts.DATA_PATH + 'train.pkl', gg_emb)
    print('Start writing val set into file.')
    write_cascade(graph_val, label_val, id2row, opts.DATA_PATH + 'val.pkl', gg_emb)
    print('Start writing test set into file.')
    write_cascade(graph_test, label_test, id2row, opts.DATA_PATH + 'test.pkl', gg_emb)
