...
def main(argv=None):
    ...
    gru_2 = keras.layers.Bidirectional(keras.layers.GRU(opts.rnn_units), merge_mode='concat')(gru_1)
    ...
    con_layer_model = keras.Model(inputs=casflow.input, outputs=casflow.layers[-4].output)
    ...
    casflow.load_weights(opts.save_dir+"best_weights.h5")
    print('===========Load success=======')

    print(opts.save_dir+"best_weights.h5")
    ...
    con1 = con_layer_model.predict_generator(test_generator)
    con2 = con_layer_model.predict_generator(val_generator)
    con_all = np.concatenate((con1, con2))
    all_true = np.concatenate((test_label, val_label))
    print(con_all.shape)

    print('piclke emb')
    pickle.dump({'con': con_all, 'true': all_true}, open(opts.DATA_PATH + 'cons.pkl', 'wb'))
