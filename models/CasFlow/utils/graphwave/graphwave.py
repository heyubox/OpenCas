...
from utils.graphwave.characteristic_functions import charac_function, \
    charac_function_multiscale
from utils.graphwave.utils.graph_tools import laplacian
...
def heat_diffusion_ind(graph, taus=TAUS, order = ORDER, proc = PROC,
    ...
    '''
    # Compute Laplacian
    a = nx.adjacency_matrix(graph, nodelist=nodes_index)
    n_nodes, _ = a.shape
...
def graphwave_alg(graph, time_pnts, taus= 'auto', 
              ...
              order=ORDER, proc=PROC, nb_filters=NB_FILTERS, nodes_index=[],
              **kwargs):
    ...
    if taus == 'auto':
        ...
        # taus = [smax]
    heat_print, _ = heat_diffusion_ind(graph, list(taus), order=order,
                                       proc=proc, nodes_index=nodes_index)
