    ...
    def __init__(self,
        ...
        self.inference_net = tf.keras.Sequential([
            ...
            keras.layers.Bidirectional(
                keras.layers.GRU(gru_units*2, return_sequences=True),
            ),
            keras.layers.Bidirectional(
        ...
        self.generative_net = tf.keras.Sequential([
            ...
            keras.layers.GRU(self.gru_units, return_sequences=True),
...
class PlanarFlowLayer1(keras.layers.Layer):
    def __init__(self,
                 z_dim,name='PlanarFlowLayer1'):
        super(PlanarFlowLayer1, self).__init__()
        self.z_dim = z_dim

        self.w = self.add_weight(name=name+'w',shape=(1, self.z_dim,), initializer='random_normal', trainable=True)
        self.u = self.add_weight(name=name+'u',shape=(1, self.z_dim,), initializer='random_normal', trainable=True)
        self.b = self.add_weight(name=name+'b',shape=(1,), initializer='random_normal', trainable=True)
    ...
    def get_config(self):
       config = {"w":self.w,"u":self.u,"b":self.b}
       base_config = super(PlanarFlowLayer1, self).get_config()
       return dict(list(base_config.items()) + list(config.items()))


class PlanarFlowLayer(keras.layers.Layer):
    def __init__(self,
                 z_dim,name='PlanarFlowLayer'):
    ...
    def get_config(self):
       config = {"w":self.w,"u":self.u,"b":self.b}
       base_config = super(PlanarFlowLayer, self).get_config()
       return dict(list(base_config.items()) + list(config.items()))
