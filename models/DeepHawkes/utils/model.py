...
class SDPP(object):
    def __init__(self, config, sess, n_nodes):
        ...
        self.initializer2 = tf.random_uniform_initializer(
        ...
        # DATA_PATH = "../weibo170w/"
        # embedding_path = DATA_PATH + '/node_vec.pkl'
        # import pickle
        # self.node_embddding = pickle.load(open(embedding_path, 'rb'))
        ...
        if self.classification:
            # both ways of calculating cost are ok

            # cost = tf.losses.sparse_softmax_cross_entropy(truth, self.pred)

            truth_onehot = tf.one_hot(truth, depth=5)
            cost = tf.nn.softmax_cross_entropy_with_logits_v2(
                truth_onehot, self.pred)
            # cost = tf.losses.softmax_cross_entropy()

            pred_soft = tf.nn.softmax(self.pred, dim=-1)
            self.preds = tf.arg_max(pred_soft, dimension=-1)
            # cost = tf.reduce_mean(tf.pow(self.pred - truth, 2)) # + self.scale*tf.add_n([self.regularizer(var) for var in tf.trainable_variables()])

            correct_prediction = tf.equal(self.preds, tf.cast(truth, tf.int64))
            acc = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
            tf.summary.scalar("acc", acc)
            error = acc
        else:
            # + self.scale*tf.add_n([self.regularizer(var) for var in tf.trainable_variables()])
            cost = tf.reduce_mean(tf.pow(self.pred - truth, 2))
        ...
        grads1 = [tf.clip_by_norm(grad, self.max_grad_norm)
                  for grad in grads[:len(var_list1)]]
        grads2 = [tf.clip_by_norm(grad, self.max_grad_norm)
                  for grad in grads[len(var_list1):]]
        ...
        elif config.fix and config.PRETRAIN:
            self.train_op = train_op1
    ...
    def build_input(self):
        ...
        self.time_interval_index = tf.placeholder(
            tf.float32, [None, self.n_time_interval], name="time")
        # (total_number of sequence,n_time_interval)
        self.rnn_index = tf.placeholder(
            tf.float32, [None, self.n_steps], name="rnn_index")
        # (total_number of sequence,n_steps)
        ...
        with tf.variable_scope(self.name) as scope:
            with tf.variable_scope('embedding'):
                if not self.PRETRAIN:
                    self.embedding = tf.get_variable('embedding', initializer=self.initializer2(
                        [self.n_nodes, self.embedding_size]), dtype=tf.float32)
                # initial pretrained embeddings and fixed it or with a smaller learning rate while training.
                else:
                    self.embedding = tf.get_variable(
                        'embedding', initializer=tf.constant(self.node_embddding, dtype=tf.float32))
            ...
            with tf.variable_scope('SumPooling'):
                self.time_weight = tf.get_variable('time_weight', initializer=self.initializer([
                                                   self.n_time_interval]), dtype=tf.float32)
            ...
            with tf.variable_scope('dense'):
                self.weights = {
                    ...
                    # 'out_class': tf.get_variable('out_weight_class', initializer=self.initializer([self.n_hidden_dense2, self.n_class]))
                }
                ...
                self.biases = {
                    ...
                    # 'out_class': tf.get_variable('out_bias_class', initializer=self.initializer([self.n_class]))
                }
                if self.classification:
                    # self.biases['out_class'] = tf.get_variable('out_bias_class', initializer=self.initializer([self.n_class]))
                    self.weights['out_class'] = tf.get_variable(
                        'out_weight_class', initializer=self.initializer([self.n_hidden_dense2, self.n_class]))
                else:
                    self.weights['out'] = tf.get_variable(
                        'out_weight', initializer=self.initializer([self.n_hidden_dense2, 1]))
                    self.biases['out'] = tf.get_variable(
                        'out_bias', initializer=self.initializer([1]))
        ...
        with tf.device('/gpu:1'):
            with tf.variable_scope('deephawkes') as scope:
                ...
                with tf.variable_scope('RNN'):
                    ...
                    # x_vector = tf.split(0, self.n_steps, x_vector) # for tf0.12
                    x_vector = tf.split(x_vector, self.n_steps, 0)  # for tf1.x
                    # outputs, _ = rnn.rnn(self.gru_fw_cell, x_vector, dtype=tf.float32) # for tf0.12
                    outputs, _ = rnn.static_rnn(
                        self.gru_fw_cell, x_vector, dtype=tf.float32)  # for tf1.x

                    # hidden_states = tf.transpose(tf.pack(outputs), [1, 0, 2]) # for tf0.12
                    hidden_states = tf.transpose(
                        tf.stack(outputs), [1, 0, 2])  # for tf1.x
                    # (total_number of sequence, n_steps, n_hidden_gru)

                    # filter according to the length
                    hidden_states = tf.reshape(
                        hidden_states, [-1, 2*self.n_hidden_gru])
                    #   (total_number of sequence*n_step, 2*n_hidden_gru)

                    rnn_index = tf.reshape(self.rnn_index, [-1, 1])
                    #   (total_number of sequence*n_step,1)

                    # hidden_states = tf.mul(rnn_index,hidden_states) # for tf0.12
                    hidden_states = tf.multiply(
                        rnn_index, hidden_states)  # for tf1.x
                    #   (total_number of sequence*n_step, 2*n_hidden_gru)

                    hidden_states = tf.reshape(
                        hidden_states, [-1, self.n_steps, 2*self.n_hidden_gru])
                    #   (total_number of sequence,n_step,2*n_hidden_gru)

                    hidden_states = tf.reduce_sum(
                        hidden_states, reduction_indices=[1])
                ...
                with tf.variable_scope('SumPooling'):
                    ...
                    time_weight = tf.matmul(
                        self.time_interval_index, time_weight)
                    #   (total_number of sequence,1)

                    # hidden_graph_value = tf.mul(time_weight,hidden_states)  #for tf0.12
                    hidden_graph_value = tf.multiply(
                        time_weight, hidden_states)  # for tf1.x
                    #   (total_number of sequence,2*n_hidden_gru)
                    self.hidden_graph_value = hidden_graph_value
                    ...
                    #                                shape=[self.batch_size, self.n_sequences, 2 * self.n_hidden_gru]) # for tf0.12
                    #
                    hidden_graph = tf.SparseTensor(indices=self.x_indict, values=hidden_graph_value,
                                                   dense_shape=[self.batch_size, self.n_sequences, 2 * self.n_hidden_gru])  # for tf1.x
                ...
                with tf.variable_scope('dense'):
                    dense1 = self.activation(
                        tf.add(tf.matmul(hidden_graph, self.weights['dense1']), self.biases['dense1']))
                    dense2 = self.activation(
                        tf.add(tf.matmul(dense1, self.weights['dense2']), self.biases['dense2']))
                    # pred = self.activation(tf.add(tf.matmul(dense1, self.weights['out']), self.biases['out']))

                    if self.classification:
                        pred = tf.matmul(dense2, self.weights['out_class'])
                    else:
                        pred = tf.add(
                            tf.matmul(dense2, self.weights['out']), self.biases['out'])
    ...
    def get_hidden_states(self, x, x_indict, y, time_interval_index, rnn_index):
        return self.sess.run([self.hidden_states], feed_dict={self.x: x, self.x_indict: x_indict, self.y: y,
                                                            self.time_interval_index: time_interval_index,
                                                            self.rnn_index: rnn_index})

    def get_hidden_graph_states(self, x, x_indict, y, time_interval_index, rnn_index):
        return self.sess.run([self.hidden_graph_value], feed_dict={self.x: x, self.x_indict: x_indict, self.y: y,
                                                            self.time_interval_index: time_interval_index,
                                                            self.rnn_index: rnn_index})
    ...
    def predict(self, x, x_indict, y, time_interval_index, rnn_index):
        return self.sess.run(self.pred, feed_dict={self.x: x, self.x_indict: x_indict, self.y: y,
                                                   self.time_interval_index: time_interval_index,
                                                   self.rnn_index: rnn_index})
    def get_pre_true(self,x, x_indict, y, time_interval_index, rnn_index):
        return self.sess.run([self.pred,self.y], feed_dict={self.x: x, self.x_indict: x_indict, self.y: y,
                                                   self.time_interval_index: time_interval_index,
                                                   self.rnn_index: rnn_index})
