    ...
    with open(flename, 'r') as f:
        for line in f:
            ...
            for i in range(1, len(walks)):
                ...
                cascade_list.append([[int(xx) for xx in s.split(",")], int(t)])
            # origin code do not need to sort
            cascade_list.sort(key=lambda x: x[1])
            graphs[walks[0]] = cascade_list
    return graphs
...
def write_XYSIZE_data(graphs, labels, sizes, LEN_SEQUENCE, NUM_SEQUENCE, index, filename):
    ...
    labelname = filename.replace('data_', 'label_')
...
def normalize_size(sizes_train, sizes_val, sizes_test, norm_file=opts.DATA_PATH+'standard.pkl'):
    ...
    pickle.dump(sizes, open(opts.DATA_PATH+'size.pkl', 'wb'))
...
if __name__ == "__main__":
    ...
    labels_train, sizes_train = read_labelANDsize(opts.cascade_train)
    ...
    pickle.dump((index.new_to_original), open(opts.DATA_PATH+"idmap.pkl", 'wb'))

    print(len(labels_train))
    # write the x,y,and size  data
    write_XYSIZE_data(graphs_train, labels_train, sizes_train, LEN_SEQUENCE, NUM_SEQUENCE, index, opts.train_pkl)
    write_XYSIZE_data(graphs_val, labels_val, sizes_val, LEN_SEQUENCE, NUM_SEQUENCE, index, opts.val_pkl)    # no val in viral
    write_XYSIZE_data(graphs_test, labels_test, sizes_test, LEN_SEQUENCE, NUM_SEQUENCE, index, opts.test_pkl)

    # write the node information
    print((len(original_ids), NUM_SEQUENCE, LEN_SEQUENCE))
    pickle.dump((len(original_ids), int(NUM_SEQUENCE), LEN_SEQUENCE), open(opts.information, 'wb'))
    ...
    if opts.PRETRAIN:
        ...
        embedding_path = opts.DATA_PATH + 'node_vec.pkl'
