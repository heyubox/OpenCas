...
def cheby_conv(x, L, lmax, batch_size, num_nodes, feat_out, K, W):
    ...
    nNode = int(num_nodes)
    # print(nSample, num_nodes, type(num_nodes), type(nSample), type(nNode))
    x = tf.reshape(x, (nSample, nNode, -1))
...
class Model(object):
    ...
    def __init__(self, config, n_node, sess):
        ...
        self.num_nodes = int(config.num_nodes)  # each sampel has num_nodes
    ...
    def _build_model(self, reuse=None):
        ...
        with tf.variable_scope('gconv_model', reuse=reuse) as sc:
            ...
            # outputs <n_step, batch, nodes, features>
            # rnn_cell_hiddens <n_step,batch,nodes,F>
            self.rnn_cell_hiddens = tf.stack(outputs)
        ...
        with tf.variable_scope('time_decay'):
            ...
            time_interval_index = tf.reshape(self.time_interval_index, [-1, self.n_time_interval])
    ...
    def train_batch(self, x, L, y, time_interval_index, rnn_index):
        ...
        # L=np.array(L)
        # y=np.array(y)
        # time_interval_index=np.array(time_interval_index)
        # rnn_index=np.array(rnn_index)
        # [self.train_op, self.time_weight]
        _, time_weight = self.sess.run([self.train_op, self.time_weight], feed_dict={self.x: x, self.laplacian: L, self.y: y, self.time_interval_index: time_interval_index, self.rnn_index: rnn_index})
    ...
    def get_rnn_hidden(self, x, L, y, time_interval_index, rnn_index):
        return self.sess.run(self.rnn_hidden, feed_dict={
            self.x: x,
            self.laplacian: L,
            self.y: y,
            self.time_interval_index: time_interval_index,
    ...
    def get_gnn_o(self, x, L, y, time_interval_index, rnn_index):
        return self.sess.run(self.rnn_cell_hiddens, feed_dict={
            self.x: x,
            self.laplacian: L,
            self.y: y,
            self.time_interval_index: time_interval_index,
