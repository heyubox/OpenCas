...
import numpy as np
import os
from utils.metrics import mape_loss_func2, mSEL, MSEL, accuracy
from utils.utils_func import ProgressBar
...
if __name__ == "__main__":
    ...
    optimizer = optim.SGD(topolstm.parameters(),
    ...
    for epoch in range(opts.epochs):
        ...
        val_mSEL_loss = []
        test_mSEL_loss = []
        for _ in tqdm(range(train_batch_number), ncols=100):
            topolstm.train()
            topolstm.zero_grad()
            sequence_matrix, sequence_mask_matrix, topo_mask_matrix, label = train_loader.__call__(
            )
            ...
            train_mSEL_loss.extend(MSEL(predict_result, label))
            train_mean_loss.append(loss.cpu().detach().numpy())
            train_acc_loss.append(accuracy(predict_result, label, 0.5))
            train_mape_loss.append(mape_loss_func2(predict_result, label))
            optimizer.step()
        train_mSEL = mSEL(train_mSEL_loss)
        train_MSEL = np.mean(train_mean_loss)
        train_acc_loss = np.mean(train_acc_loss)
        train_mape = np.mean(train_mape_loss)
        for _ in tqdm(range(val_batch_number), ncols=100):
            topolstm.eval()
            topolstm.zero_grad()
        ...
        if best_val > val_MSEL:
            torch.save(topolstm.state_dict(), opts.save_dir+'save_{}.pt'.format(opts.observation_time))
            best_val = val_MSEL
        for _ in tqdm(range(test_batch_number), ncols=100):
            topolstm.eval()
            topolstm.zero_grad()
        ...
        if best_test > test_MSEL:
            best_test = test_MSEL
        print(
            '# Epochs:{}\nTrain Loss MSEL:{:5f}, Val Loss MSEL:{:5f}, Test Loss MSEL:{:5f},'
            .format(epoch, train_MSEL, val_MSEL, test_MSEL))
        print(
            'Train Loss mSEL:{:5f}, Val Loss mSEL:{:5f}, Test Loss mSEL:{:5f},'
            .format(train_mSEL, val_mSEL, test_mSEL))
        print(
            'Train acc:{:5f}, Val acc:{:5f}, Test acc:{:5f},'
            .format(train_acc_loss, val_acc_loss, test_acc_loss))
        print(
            'Train Loss mape:{:5f}, Val Loss mape:{:5f}, Test Loss mape:{:5f},\n'
            .format(train_mape, val_mape, test_mape))
print(
    '# BEST, Val Loss MSEL:{:5f}, Test Loss MSEL:{:5f},'
    .format(best_val, best_test))
