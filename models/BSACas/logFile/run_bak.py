# from process_cascade import qualified_cascade, parse_line, get_observation_path, parse_observation_path, parse_sequence_counts
...
from utils.utils_func import seed_everything, sparse_mx_to_torch_sparse_tensor, atten_mx, atten_self_mx, gen_ptr, array2tensor, gen_index, sensitive_mse
import sys
import math
...
def model_train(model, optimizer, X_train, Y_train, batch_size, n_seq=None, device=None, load_concat=False, epoch=0):
    ...
    loss_total = []
    print('total batch: {} '.format(len(X_train) / batch_size))
    ...
    for i in range(math.ceil(len(X_train) / batch_size)):
        ...
        if (i + 1) % 100 == 0:
            print('batch:{} average train loss {:.04f}'.format(i, sum(loss_total[-5000:]) / len(loss_total[-5000:])))
    ...
    # r2score = r2_score(Y_trues[Y_preds > 0], Y_preds[Y_preds > 0])
    # print('r2score train {:.04f}'.format(r2score))
...
if __name__ == '__main__':
    ...
    model = GRU_Cascade_Dynamic_GPN(n_seq=opts.n_seq,
    ...
    for epoch in range(epochs):
        ...
        if epoch == 20:
            optimizer = torch.optim.Adam(model.parameters(), lr=opts.learning_rate, weight_decay=1e-5, momentum=0.9)
    ...
    loss_dict = {"train": train_los_list, "val": val_loss_list, "test": test_loss_list, 'text': " loss = loss_+ loss_diff epoch == 20 change adam l2"}
    save_num = 0
